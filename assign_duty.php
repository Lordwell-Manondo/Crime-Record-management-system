...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $is_valid = true;
    if (empty($_POST['case_serial_no']) || empty($_POST['emp_number']) || empty($_POST['date_to_report'])) {
    ...
    if (!is_string($case_serial_no) || !is_string($emp_number)) {
    ...
    if ($is_valid) {
        // Prepare SQL query
        $sql = "INSERT INTO duty (case_serial_no, emp_number, date_to_report)
            VALUES ('$case_serial_no', '$emp_number', '$date_to_report')";
...
$sql = "SELECT * FROM `cases` ORDER BY serial_no";
...
if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_assoc($result)) {
    $officers.="<option value='" . $row['employee_number'] . "'>".$row['id']." (" . $row['first_name'] .' '.$row['last_name'] . ")</option>";
  }
}
